 ...
 * Function: 使用{@link LruMemoryCache}保存图片的引用
 */

public class ComparePresenter extends BasePresenter<CompareContract.View> implements
        CompareContract.Presenter {
    private boolean threadFlag;
    private boolean compareFlag;
    private Disposable mCountDisposable;
    private Subscription mSubscription;
    private LruMemoryCache mMemoryCache = App.getInstance().getLruMemoryCache();

    @Override
    public void compare(final int countInterval, final float threshold) {
//        startCountDown(countInterval);
//        threadFlag = true;
        ...
        Flowable.interval(0, 500, TimeUnit.MILLISECONDS)
                ...
                .flatMap(new Function<Long, Publisher<Compare>>() {
                    ...
                    public Publisher<Compare> apply(@NonNull Long aLong) throws Exception {
                        ...
                        Map.Entry<String, Bitmap> entry = mView.takePicture();
                        if (entry == null) {
                            return Flowable.empty();
                        }
                        Compare compare = new Compare();
                        compare.setBitmap(entry.getValue());
